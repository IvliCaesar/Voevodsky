...
% Adequate equivalence relations / regular homomorphisms
...
\noindent
The author would like to thank Dustin Clausen for illuminating conversations,
and Chuck Weibel for his interest in this work.
\newpage
...
It is worth pausing on why convergence is not automatic.  The spectral
sequence~\eqref{eq:SS} is built, exactly as in the classical Postnikov-tower or
Adams spectral sequence construction in stable homotopy theory
(cf.\ \cite{Boa99}), from an \emph{infinite} tower of triangles
$bc_{\leq p-1}A\to bc_{\leq p}A\to bc_{p/p-1}A$ indexed by $p\in\Z$, rather than
from a bounded or even bounded-below filtration.  Convergence of such spectral
sequences is a subtle phenomenon: the individual pages $E_r^{p,q}$ may
fail to stabilise, the abutment of the tower (its homotopy colimit) need not agree
with the object one started with, and even when the pages do stabilise the
resulting filtration on $\Hom_{DM}(B,A)$ may fail to be exhaustive, separated, or
complete.  Boardman's formalism (recalled in
\S\ref{subsec:spectral-sequences}) isolates exactly the two obstructions that must
vanish: the map $c:\hocolim_n bc_{\leq n}A\to A$ from the homotopy colimit of the
tower back to $A$ must be an isomorphism (this controls exhaustiveness of the
abutment), and the derived limit term $RE_\infty^{p,q}$ must vanish (this controls
completeness).  The bulk of this thesis — Theorem~\ref{thm:main} and its
proof via Corollary~\ref{cor:hocolim-iso} and
Theorem~\ref{thm:DM-strong-convergence} — is devoted to establishing both facts
for $DM$, using in an essential way that the source object $B$ is a shifted Tate
twist $M(X)(r)[s]$ of the motive of a smooth scheme, so that $B$ is both compact
and orthogonal to $DM^{\mathrm{eff}}(n)$ for $n$ large (Proposition~\ref{prop:orthogonality}),
which forces the spectral sequence into a half-plane and makes the derived limit
term vanish automatically (Note~\ref{note:tower-halfplane}).
...
The mechanism behind this failure is instructive: the argument that makes
$M(X)(r)[s]$ well behaved in $DM$ uses that a motive of a $d$-dimensional scheme is
automatically orthogonal to $DM^{\mathrm{eff}}(n)$ once $n>d+r$ — a rigidity coming from
the vanishing of motivic cohomology in negative weights.  No such rigidity holds in
$\SH$: the unit object $\mathbf{1}\in\SH$ has nonzero slices $s_m\mathbf{1}$ in
every degree $m\geq 0$ (Voevodsky, \cite{Voe02a}), so $\mathbf{1}$ is never
orthogonal to $\SH^{\mathrm{eff}}(n)$ for large $n$, and the orthogonal tower on
$\mathbf{1}$ never stabilises.  On the other hand, we verify that, under
appropriate conditions on $A\in\SH$ — precisely, that $A$ already lies in some
$\SH^\perp(r)$ — the spectral sequence is strongly convergent
...
spectral sequence (Corollary~\ref{cor:sliceconv}).  Thus the orthogonal spectral
sequence is well behaved precisely on the pieces that the slice filtration itself
produces, even though it is ill behaved on the ambient category $\SH$.

\paragraph{Outline.}  Section~\ref{sec:preliminaries} collects the background
material used throughout: triangulated categories, Bousfield localisation and
Neeman's adjoint functor theorem (\S\ref{sec:preliminaries}.1–2), the language of
exact couples and Boardman's convergence criteria for spectral sequences
(\S\ref{subsec:spectral-sequences}), and the construction of the triangulated
category of mixed motives $DM$ together with Voevodsky's slice tower
(\S\ref{sec:preliminaries}.5).  Readers already familiar with this material may
proceed directly to Section~\ref{sec:results}, which contains the new results of
the thesis: the construction of the orthogonal tower and its associated spectral
sequence (\S\ref{subsec:orthogonal-tower}), the proof of strong convergence in
$DM$ culminating in Theorem~\ref{thm:main} (\S\ref{subsec:convergence-tower}), and
the comparison with the $\mathbb{A}^1$-stable homotopy category $\SH$, including
the counterexample of Proposition~\ref{prop:counterexample} and the positive
result for slices, Corollary~\ref{cor:sliceconv}
(\S\ref{subsec:SH}).  Section~\ref{subsec:applications} surveys three subsequent
applications of the orthogonal and slice towers, due to Peláez and
collaborators, to the classical theory of adequate equivalence relations on
algebraic cycles, and the thesis closes with further results, worked examples,
and a discussion of the problems that remain open in \S\ref{s.openq}.

\begin{note}
The main results of this thesis (Theorem~\ref{thm:main} and its consequences)
were subsequently published as \cite{GalPel22}.
\end{note}
...
\begin{remark}
A distinguished triangle $X\xrightarrow{u}Y\xrightarrow{v}Z\xrightarrow{w}X[1]$
should be thought of as an abstraction of a cofiber sequence
$X\to Y\to Y/X$ in topology, or of a short exact sequence $0\to X\to Y\to Z\to 0$
in an abelian category, with $Z$ playing the role of the ``cone'' or
``cokernel'' of $u$ and the connecting map $w:Z\to X[1]$ recording the failure of
$u$ to be an honest monomorphism.  Axiom~\ref{TR1}(c) guarantees that every
morphism has a cone; axiom~\ref{TR3} is the analogue of the snake lemma, producing
a (non-unique, in general) comparison map on cones; and the octahedral
axiom~\ref{TR4} governs how cones of composites relate to iterated cones, playing
the role played by the third isomorphism theorem for quotients in an abelian
category.  Throughout this thesis the triangulated category $DM$ (constructed in
\S\ref{sec:preliminaries}.5) should be kept in mind as the running example: its
distinguished triangles come from mapping cones of complexes of sheaves with
transfers, and $\Hom_{DM}(-,-)$ plays the role of the derived Hom of sheaf theory.
\end{remark}
...
\begin{remark}\label{rem:Bousfield-intuition}
Proposition~\ref{prop:Bousfield-triangle} says that every object $T$ decomposes,
functorially and up to a distinguished triangle, into an ``$\mathcal{S}$-torsion
part'' $T_\mathcal{S}\in\mathcal{S}$ and an ``$\mathcal{S}$-local part'' $GT$ that
sees nothing of $\mathcal{S}$ (Note~\ref{note:S-local}).  This is the exact
categorical analogue of the decomposition of an abelian group into its torsion
subgroup and its torsion-free (localised) quotient, or of the arithmetic
fracture square decomposing a spectrum into its rationalisation and its
completions at each prime.  The orthogonal tower construction of
Definition~\ref{def:orthogonal-cover} below is built from exactly this
machinery: at each stage $n$ we take $\mathcal{S}=DM^{\mathrm{eff}}(n+1)$, so that
$bc_{\leq n}E=j_{n+1}p_{n+1}E$ is the ``$DM^{\mathrm{eff}}(n+1)^\perp$-local
part'' of $E$, i.e.\ the part of $E$ that is orthogonal to (does not see maps
from) motives of Tate twist $\geq n+1$.  Increasing $n$ enlarges the class of
motives being tested against, so $bc_{\leq n}E$ should be thought of as an
increasingly fine approximation to $E$, in analogy with a Postnikov tower
approximating a spectrum by killing higher and higher homotopy.
\end{remark}
...
\begin{remark}
Concretely, $E_{r+1}^{pq}=Z_r^{pq}/B_r^{pq}$ is obtained from $E_r^{pq}$ by
discarding the elements killed by $d_r$ going out (these are not ``cycles'') and
further quotienting by the elements hit by $d_r$ coming in (these are
``boundaries'').  As $r$ grows, both the differentials $d_r$ and their sources and
targets move further apart in bidegree, so in any \emph{fixed} bidegree $(p,q)$ the
groups $E_r^{pq}$ eventually stabilise once $r$ is larger than the maximum of the
distances to the nearest possibly-nonzero neighbouring group — this is the case,
for instance, whenever the spectral sequence is confined to a half-plane
(Note~\ref{note:tower-halfplane}).  The subtlety addressed by
Definition~\ref{def:infinite-terms} and the convergence criteria of
\S\ref{subsubsec:exact-couples}--\S\ref{subsubsec:tower-SS} is that ``eventually
stable in each bidegree'' is \emph{not} automatically the same as ``correctly
computes the associated graded of the abutment'': the two can differ by the
derived limit term $RE_\infty^{pq}$ of Lemma~\ref{lem:Einf-and-filtration}, which
is why strong convergence requires an extra hypothesis beyond mere degreewise
stabilisation.
\end{remark}
...
\begin{remark}[Comparison with the slice tower]\label{rem:orthogonal-vs-slice}
It is worth contrasting the orthogonal tower $\{bc_{\leq n}\}$ of
Definition~\ref{def:orthogonal-cover} with the slice tower $\{f_n\}$ of
Definition~\ref{def:slice-tower}, since both arise from the same mechanism —
Neeman's adjoint functor theorem (Theorem~\ref{thm:Neeman}) applied to a nested
family of localising subcategories — but run in opposite directions.  The slice
functor $f_n=i_n\circ r_n$ is built from the inclusion
$i_n:DM^{\mathrm{eff}}(n)\hookrightarrow DM$ of the subcategory \emph{generated}
by Tate twists of weight $\geq n$, followed by its right adjoint $r_n$; thus
$f_nE$ retains only the part of $E$ visible to \emph{high}-weight test objects,
and $f_n(\Z(n))\cong\Z(n)$ while $f_m(\Z(n))=0$ once $m$ exceeds $n$
(Proposition~\ref{prop:slice-Z}).  The orthogonal cover
$bc_{\leq n}=j_{n+1}\circ p_{n+1}$ is built instead from the inclusion
$j_{n+1}:DM^\perp(n+1)\hookrightarrow DM$ of the subcategory \emph{orthogonal to}
weight $\geq n+1$ test objects, so $bc_{\leq n}E$ retains the part of $E$ that
cannot be detected by high-weight probes.  Both towers stabilise on a Tate motive
$\Z(m)[r]$ — the slice tower because $\Z(m)[r]$ eventually leaves
$DM^{\mathrm{eff}}(n)$ as $n$ grows past $m$, the orthogonal tower because
$\Z(m)[r]$ enters $DM^\perp(n+1)$ once $n\geq m$
(Example~\ref{ex:bc-Tate}) — but for a general object, and especially for
objects of $\SH$ such as the sphere spectrum $\mathbf 1$, the two towers behave
very differently: Proposition~\ref{prop:counterexample} below shows the
orthogonal tower on $\mathbf 1\in\SH$ never stabilises, precisely because
$\mathbf 1$ has nonzero slices $s_m\mathbf 1$ in every weight $m\geq 0$, so it is
never orthogonal to $\SH^{\mathrm{eff}}(n+1)$ for any $n$.
\end{remark}

% ------------------------------------------------------------------
\subsection{Convergence of the orthogonal tower}
\label{subsec:convergence-tower}
% ------------------------------------------------------------------
...
\begin{example}\label{ex:bc-Tate}
Let $Y=\operatorname{Spec}k$, so that $d=\dim Y=0$, and let $m,r\in\Z$.  Applying
Corollary~\ref{cor:bc-isomorphism}(1) to $E=\Z(m)[r]=M(\operatorname{Spec}k)(m)[r]$
shows that the counit
\[
  \theta_m^{\Z(m)[r]}:bc_{\leq m}\bigl(\Z(m)[r]\bigr)\longrightarrow\Z(m)[r]
\]
is already an isomorphism in $DM$.  In other words, the orthogonal cover of a
(shifted) Tate motive recovers it on the nose at stage $n=m$ of the tower — no
information is lost by truncating there.  This is the simplest instance of the
mechanism exploited in the proof of Theorem~\ref{thm:DM-strong-convergence}: for
$B=M(X)(s)[r]$ of dimension $d=\dim X$, the orthogonal cover $bc_{\leq d+s}A$
already computes $\Hom_{DM}(B,A)$ through $\theta_{d+s}^A$, and it is exactly this
finite stage of stabilisation, uniform in $A$, that forces the vanishing
$E_1^{p,q}=0$ for $p\leq s-1$ used there.
\end{example}
...
\begin{remark}[Significance for the Bloch--Beilinson--Murre filtration]
\label{rem:BBM-significance}
Recall from the Introduction that the motivation for the orthogonal tower is
Peláez's construction of a finite filtration on the Chow groups $\CH^q(Y)_R$,
obtained by evaluating $bc_{\leq\bullet}$ on the motive of a point and mapping in
$M(Y)(-q)[-2q]$.  Theorem~\ref{thm:main} shows that, for \emph{every}
$B=M(X)(r)[s]$ — not merely the specific twist used to define the Chow-group
filtration — the orthogonal spectral sequence converges strongly.  Consequently
the associated filtration $F_\bullet$ on $\Hom_{DM}(B,A)$
(Definition~\ref{def:filtration-F}) is automatically exhaustive, separated, and
complete for every smooth $X$ and every twist and shift, so that
$\Hom_{DM}(B,A)$ is recovered from the tower, up to the extension problem
recorded by the $E_\infty$-page.  This is precisely the convergence statement
needed before properties expected of a Bloch--Beilinson--Murre filtration — such
as finiteness of the graded pieces $\Gr_F^pH$ — can be meaningfully transported
from statements about the individual layers $bc_{p/p-1}A$ to statements about
$\Hom_{DM}(B,A)$ itself: without strong convergence, the filtration could fail to
be exhaustive, or the comparison between $E_\infty^{p,q}$ and
$\Gr_F^p\Hom_{DM}(B,A)$ could carry an uncontrolled discrepancy
(Lemma~\ref{lem:Einf-and-filtration}).  Theorem~\ref{thm:main} removes this
obstruction unconditionally, without assuming any part of the (still
conjectural) Bloch--Beilinson--Murre programme itself.
\end{remark}
...
% ------------------------------------------------------------------
\subsection{Further applications: Chow correspondences, regular
homomorphisms, and incidence equivalence}
\label{subsec:applications}
% ------------------------------------------------------------------

The orthogonal tower $\{bc_{\leq n}\}$ and the slice tower $\{f_n\}$ studied
above are not only convergence-theoretic tools: since the publication of
Theorem~\ref{thm:main} as \cite{GalPel22}, they have been used by Peláez and
collaborators to give purely motivic characterisations of several classical
\emph{adequate equivalence relations} (in the sense of Samuel~\cite{Sam60}) on
algebraic cycles.  We summarise three such applications, each of which sharpens
the picture of the Bloch--Beilinson--Murre filtration discussed in
\S\ref{s.openq} below and uses, verbatim, the towers $bc_{\leq n}$ and $f_n$
constructed in \S\ref{subsec:orthogonal-tower} and
\S\ref{subsubsec:postnikov} respectively.  We state the results with the
notation of this thesis; full proofs are in the cited sources.

\subsubsection{The orthogonal cover detects vanishing of regular homomorphisms
into abelian varieties}

Recall (Lieberman~\cite{Lie68}) that a group homomorphism
$\psi:\CH^n_{\alg}(X)\to A(k)$, for $A$ an abelian variety, is a \emph{regular
homomorphism} if it is induced, family-wise, by morphisms of varieties; the
prototypical example is Weil's Abel--Jacobi map.  Given $X,Y\in\SmProj_k$ with
$X$ of dimension $d$, a base point $x_0\in X(k)$, and a Chow correspondence
$\Lambda\in\CH^{d+n}(Y\times X)$, one obtains a cycle-induced regular
homomorphism $\psi:\CH^{d_Y-n}(Y)\to\Alb(X)(k)$,
$\gamma\mapsto\mathrm{alb}_X(\Lambda(\gamma)-\deg(\Lambda(\gamma))[x_0])$, into
the Albanese variety of $X$.

\begin{theorem}[Peláez~\cite{Pel25}]\label{thm:app-chowcor}
Let $\underline{\Alb}(X)\in DM$ be the functor of points of $\Alb(X)$ and
$\mathcal A_f:M(X)\to\underline{\Alb}(X)$ the map classifying the canonical
morphism $X\to\Alb(X)$ sending $x_0$ to $0$.  Dualising $M(X)$ gives
$\Lambda_X:M(Y)(-n)[-2n]\to M(X)$ from the correspondence $\Lambda$.  Then a
lift $\Lambda'$ of $\mathcal A_f\circ\Lambda_X$ through the orthogonal cover
$\theta_{-1}^{\underline{\Alb}(X)}:bc_{\leq -1}(\underline{\Alb}(X))\to
\underline{\Alb}(X)$ exists in $DM$ \emph{if and only if} $\psi\equiv 0$.
\end{theorem}
\begin{proof}[Proof (sketch, following~\cite{Pel25})]
By the universal property of the counit $\varepsilon_0:f_0\to\mathrm{id}$ of
the \emph{slice}-tower adjunction (\cite{Pel17}, 5.3.2 — the analogue, for
the source object, of the universal property of $\theta_{-1}$ used in
Proposition~\ref{prop:bc-properties}(2) for the target), a lift $\Lambda'$
through $\theta_{-1}^{\underline{\Alb}(X)}$ exists if and only if the
composite
\[
  f_0\bigl(M(Y)(-n)[-2n]\bigr)
  \xrightarrow{\varepsilon_0}
  M(Y)(-n)[-2n]
  \xrightarrow{\Lambda_X}
  M(X)
  \xrightarrow{\mathcal A_f}
  \underline{\Alb}(X)
\]
is zero in $DM$.  Since $\mathbf 1\in\DMeff$, the counit
$\varepsilon_{0\ast}$ is an isomorphism on $\Hom_{DM}(\mathbf 1,-)$ by the
same universal property, so the displayed composite vanishes if and only if
it vanishes after applying $\Hom_{DM}(\mathbf 1,-)$.  Under the
identifications $\Hom_{DM}(\mathbf 1,M(Y)(-n)[-2n])\cong\CH^{d_Y-n}(Y)$ and
$\Hom_{DM}(\mathbf 1,\underline{\Alb}(X))\cong\Alb(X)(k)$, the map
$\Lambda_X$ induces $\gamma\mapsto\Lambda(\gamma)$ (Poincaré duality) and
$\mathcal A_f$ induces $\alpha\mapsto\mathrm{alb}_X(\alpha-\deg(\alpha)[x_0])$
(naturality of the Albanese map, using
$\CH^d(X)\cong\CH^d_{\alg}(X)\oplus\Z$); composing these two identifications
is exactly $\psi$.  Hence the displayed composite vanishes on
$\Hom_{DM}(\mathbf 1,-)$ if and only if $\psi\equiv 0$, proving the theorem.
\end{proof}

Two immediate corollaries make this criterion
effective: if $\Lambda$ is algebraically equivalent to zero \emph{on both
factors} (``square equivalent to zero''), or — over $k=\mathbb{C}$ — lies in the
second step of H.\ Saito's filtration~\cite{Sai79}, then the lift through
$bc_{\leq-1}$ automatically exists, i.e.\ $\psi\equiv 0$ automatically.  Thus
the single orthogonal cover $\theta_{-1}$ — the very map whose stabilisation
(Corollary~\ref{cor:bc-isomorphism}) drives the proof of
Theorem~\ref{thm:main} — already detects two classically defined equivalence
relations at once.

\subsubsection{A motivic model for the kernel of an arbitrary regular
homomorphism}

Peláez~\cite{HerPel25} gives a construction, using the slice
tower $\{f_n\}$ of Definition~\ref{def:slice-tower} rather than the orthogonal
tower, that produces a motivic model for the kernel of \emph{any} regular
homomorphism, not only one induced by a fixed correspondence.

\begin{theorem}[Peláez~\cite{HerPel25}]\label{thm:app-abequiv-1}
Let $X\in\SmProj_k$ of dimension $d$ and $\psi:\CH^n_{\alg}(X)\to A(k)$ a
regular homomorphism.  There exist $M^n_\psi(X)\in\DMeff$ and a map
$z_\psi:M^n_\psi(X)\to M(X)$, built from the distinguished triangle
$f_{d-n+1}M(X)\to f_{d-n}M(X)\to s_{d-n}M(X)$ of
Definition~\ref{def:slice-tower} and a factorisation through the slice
$s_{d-n}M(X)$ dictated by $\psi$, such that under
$\Hom_{DM}(\Z(d-n)[2d-2n],M(X))\cong\CH^n(X)$ the image of $z_{\psi\ast}$ is
exactly $\ker(\psi)\subseteq\CH^n_{\alg}(X)$.  Moreover, for every $r\geq 1$
the induced map on slices $s_{d-n+r}(z_\psi):s_{d-n+r}(M^n_\psi(X))\to
s_{d-n+r}(M(X))$ is an isomorphism, i.e.\ $M^n_\psi(X)$ differs from $M(X)$
only in the slice at $d-n$.
\end{theorem}
\begin{proof}[Proof (sketch, following~\cite{HerPel25})]
Let $\mathcal T:=\bigoplus M(Y)$, summed over all triples $(\Lambda,y_0,Y)$
with $Y\in\SmProj_k$, $\dim Y\geq1$, $\Lambda\in\CH^n(Y\times X)$,
$y_0\in Y(k)$; each summand carries a map
$\Lambda_{y_0}:M(Y)\to M(X)(-d+n)[-2d+2n]$ dual to $\Lambda$, and a map
$\mathcal A_{\psi_{\Lambda,y_0}}:M(Y)\to\underline A$ induced by $\psi$
itself.  By the universal property of the slice-tower counit
$\varepsilon_0$ applied to $\varphi_{d-n}(M(X)):=f_0(M(X)(-d+n)[-2d+2n])$,
the map $(\Lambda_{y_0})_\bullet:\mathcal T\to
M(X)(-d+n)[-2d+2n]$ factors uniquely through a map
$p:\mathcal T\to\varphi_{d-n}(M(X))$, and
$\Hom_{DM}(\mathbf 1,\varphi_{d-n}(M(X)))\cong\CH^n(X)$ with the image of
$p_\ast$ equal to $\CH^n_{\alg}(X)$ (the effective cover $f_0$ discards
exactly the part of $\CH^n(X)$ not algebraically equivalent to zero).
Completing $p$ to a distinguished triangle $\mathcal R\to\mathcal
T\xrightarrow{p}\varphi_{d-n}(M(X))$ and applying the homotopy
$t$-structure, one finds $\mathcal T,\varphi_{d-n}(M(X))\in(\DMeff)_{\geq0}$,
so $\mathcal R\in(\DMeff)_{\geq-1}$ with $\mathbf h_{-1}(\mathcal R)$ a
birational sheaf.  Since $\underline A$ is concentrated in homotopy degree
$0$, the composite $(\mathcal A_{\psi_{\Lambda,y_0}})\circ r:\mathcal
R\to\underline A$ factors through the truncation
$\mathbf h_{-1}(\mathcal R)[-1]$, defining $e_\psi$; the octahedral axiom
applied to the resulting commutative square then produces
$M^n_\psi(X)$ (a fibre of a further factorisation of $p$ through the
cofibre of $e_\psi$) together with $z_\psi:M^n_\psi(X)\to M(X)$, such that
exactness of $\Hom_{DM}(\mathbf 1,-)$ on the resulting triangle identifies
the image of $z_{\psi\ast}$ with $\ker(\psi)\subseteq\CH^n_{\alg}(X)$ — by
construction $\mathcal T$ tests \emph{every} correspondence-induced instance
of $\psi$ at once.  The slice-agreement statement follows because the
cofibre controlling the discrepancy between $M^n_\psi(X)$ and $M(X)$ is
(like $\mathbf h_{-1}(\mathcal R)$) a birational sheaf, hence has vanishing
higher Nisnevich cohomology and so satisfies $f_1(-)\cong0$; applying $f_m$
to the defining triangle then shows $f_{d-n+r}(z_\psi)$ is an isomorphism
for every $r\geq1$.
\end{proof}

Specialising $\psi$ recovers motivic models for several classical
equivalence relations: the Abel--Jacobi kernel $\CH^n_{AJ}(X)$ (taking
$\psi$ to be Weil's Abel--Jacobi map, so $A=J^n_a(X)$ over $k=\mathbb{C}$); the
incidence kernel $\CH^n_\inc(X)$ (taking $\psi$ to be the Néron--Severi
regular homomorphism onto $\Pic^n(X)$ of Grothendieck--Kleiman and H.\
Saito~\cite{Sai79}); and, applying the same construction to the product
$\underline{\mathfrak{Ab}}=\prod_{\psi,A}\underline A$ ranging over \emph{all}
regular homomorphisms out of $\CH^n_{\alg}(X)$, a motivic model $M^n_{\ab}(X)$
computing the subgroup $\CH^n_{\ab}(X)\subseteq\CH^n_{\alg}(X)$ of cycles
\emph{abelian equivalent to zero} (Samuel~\cite{Sam60}) — the intersection of
the kernels of every regular homomorphism from $X$ into every abelian
variety — with the same slice-agreement property above degree $d-n$.

\subsubsection{Computing the second step of the orthogonal filtration:
incidence equivalence}

Recall from the Introduction that evaluating the orthogonal tower
$bc_{\leq\bullet}$ on $\Z_R$ and mapping in $M(X)(-n)[-2n]$ produces the
filtration $F^\bullet\CH^n(X)_R$ used to approach the Bloch--Beilinson--Murre
conjectures — this is precisely the filtration $F_\bullet$ of
Definition~\ref{def:filtration-F} applied to $B=M(X)(-n)[-2n]$, $A=\Z_R$.  By
Theorem~\ref{thm:main} this filtration is unconditionally strongly convergent;
Peláez~\cite{Pel17} further shows (Theorem~\ref{thm:Pel-BBM1235} above) that it
satisfies BBM1, BBM2, BBM3, BBM5.  Peláez~\cite{PelRey25} computes its
\emph{second step} $F^2$ explicitly, in terms of Griffiths' incidence
equivalence~\cite{Gri71}: $\alpha\in\CH^n(X)_R$ is \emph{incidence equivalent
to zero} if $\alpha\in\CH^n_{\alg}(X)_R$ and
$\pi_{Y\ast}((\pi_X^\ast\alpha)\cdot\beta)=0\in\CH^1(Y)_R$ for every
$Y\in\SmProj_k$ and every $\beta\in\CH^{d-n+1}(X\times Y)_R$.

\begin{theorem}[Peláez~\cite{PelRey25}]\label{thm:app-incequiv}
Let $X\in\SmProj_k$ of dimension $d$.
\begin{enumerate}
\item For \emph{zero-cycles} ($n=d$) and an arbitrary base field $k$:
  $F^2\CH^d(X)_\Q=\CH^d_\inc(X)_\Q$; if moreover $k$ is algebraically closed,
  this equals the Albanese kernel $T(X)_\Q$.
\item For $\alpha\in\CH^n_{\alg}(X)_\Q$, $\beta\in\CH^m_{\alg}(Y)_\Q$, both the
  exterior product $\alpha\otimes\beta$ and (when $Y=X$) the intersection
  product $\alpha\cdot\beta$ lie in $F^2$ of the corresponding Chow group.
\item If $k$ is finite or the algebraic closure of a finite field, the same
  identification $F^2\CH^n_{\alg}(X)_\Q=\CH^n_\inc(X)_\Q$ holds in
  \emph{every} codimension $n$, not only $n=d$.
\end{enumerate}
\end{theorem}
\begin{proof}[Proof (sketch, following~\cite{PelRey25})]
For part (1): if $d=1$, $\CH^1_\inc(X)_\Q=0$ (a divisor pairs nontrivially
against the diagonal unless it already vanishes), and $F^2\CH^1(X)_\Q=0$
follows directly from Theorem~\ref{thm:Pel-BBM1235}, so both sides agree.
For $d\geq2$: the inclusion $F^2\subseteq\CH^d_\inc(X)_\Q$ is the
functoriality of $F^\bullet$ (Theorem~\ref{thm:Pel-BBM1235}) together with
the defining vanishing property of the tower.  For the reverse inclusion,
field-extension compatibility lemmas (analogous in spirit to the reduction
to a perfect, then algebraically closed, field used in
Proposition~\ref{prop:orthogonality}) reduce to $k$ algebraically closed.
If $k$ is not the algebraic closure of a finite field, the theory of
divisorial correspondences identifies the vanishing condition defining
$\CH^d_\inc(X)_\Q$ with membership in the Albanese kernel, and one checks
$\alpha\in F^2$ directly by dimension induction (curves, threefolds, and
surfaces are treated as separate base cases).  If $k$ is the algebraic
closure of a finite field, algebraic and homological equivalence coincide on
zero-cycles, so $\alpha$ additionally satisfies a second vanishing
condition studied by Bloch (a $K_1$-analogue of incidence equivalence,
pairing against $\CH^{d-n+1}(X\times Y,1)_R$ rather than
$\CH^{d-n+1}(X\times Y)_R$), and one concludes via the vanishing of the
relevant $K$-cohomology of finite fields.
The identification $\CH^d_\inc(X)_\Q=T(X)_\Q$ for $k$ algebraically closed
is the classical fact that the vanishing pairing defining incidence
equivalence characterises the Albanese kernel on zero-cycles.

For part (2): reduce to $k$ algebraically closed, write $\alpha,\beta$ as
pushforwards along correspondences $\Lambda_\alpha,\Lambda_\beta$ from
zero-cycles $\gamma_\alpha,\gamma_\beta$ on curves $C,C'$ representing the
Jacobians through which $\alpha,\beta$ factor; functoriality of $F^\bullet$
reduces the exterior product $\alpha\otimes\beta$ to
$\gamma_\alpha\otimes\gamma_\beta\in\CH^2_{\alg}(C\times C')_\Q$, which lies
in $F^2$ by part (1) once one checks it lies in the Albanese kernel of
$C\times C'$ — which follows from rigidity, since the exterior product of
Jacobian points factors through $J(C)(k)\times\{0\}$ and
$\{0\}\times J(C')(k)$, on which the Albanese map of $C\times C'$ vanishes.
The intersection product statement follows by pulling back along the
diagonal $\Delta_X:X\to X\times X$ and using functoriality again.

Part (3) follows the same reduction to zero-cycles as part (1), but with
the extra arithmetic input that over the algebraic closure of a finite
field the vanishing of Bloch's pairing (rather than only the incidence
pairing) is already implied by algebraic equivalence, so the two
characterising conditions of part (1) become available in every
codimension, not only for zero-cycles.
\end{proof}

Since the filtration $F^\bullet$ of Theorem~\ref{thm:app-incequiv} is the same
filtration whose strong convergence is Theorem~\ref{thm:main}, this result
gives an unconditional, classically meaningful description of the second
graded piece $\Gr_F^1=F^1/F^2$ — using that $F^1$ itself is identified with
numerical equivalence (\cite{Pel17}, 6.1.4, recalled as BBM2 in
Theorem~\ref{thm:Pel-BBM1235}; see also~\cite{PelRey25}) — sitting between
numerical and incidence equivalence.  It is complementary to, rather than a
resolution of, the BBM4/BBM4a discussion of \S\ref{s.openq}: it does not touch
Peláez's Conjecture~\ref{conj:Pel-multiplicative} on the multiplicativity of
the tower, but it strengthens the case that $F^\bullet$ is a reasonable
candidate for a Bloch--Beilinson filtration, since its first two steps are
now \emph{both} identified with classical, independently defined equivalence
relations (numerical, respectively incidence, equivalence) rather than only
characterised abstractly via $bc_{\leq\bullet}$.

\begin{example}
Take $k=\mathbb{C}$ and $X=E\times E'$ a product of two elliptic curves, so
$d=2$ and $\Alb(X)=E\times E'$.  By Theorem~\ref{thm:app-incequiv}(1),
$F^2\CH^2(X)_\Q=T(X)_\Q$, the Albanese kernel of $X$.  Since
$H^0(X,\Omega^2_X)\cong H^0(E,\Omega^1_E)\otimes H^0(E',\Omega^1_{E'})\neq0$,
Mumford's theorem~\cite{Mum69} — recalled at the very start of this thesis
as the original motivation for the Bloch--Beilinson--Murre programme —
shows that $T(X)_\Q$ is not finitely generated: it is ``enormous,'' with no
algebraic parametrisation.  Thus the second step of the very filtration
whose strong convergence is Theorem~\ref{thm:main} is already, for this
simple $X$, a group with no hope of being finitely generated.  This is a
concrete illustration of why the Bloch--Beilinson--Murre programme asks for
a filtration that is exhaustive, separated, and complete
(Theorem~\ref{thm:main}) — recovering $\CH^n(X)$ up to extensions from
possibly infinite graded pieces — rather than for a single Néron-Severi-type
finiteness statement, which Mumford's theorem already rules out beyond
codimension $1$.
\end{example}

% ==================================================================
\section*{Further results, examples, and open problems}
\label{s.openq}
\addcontentsline{toc}{section}{Further results, examples, and open problems}
% ==================================================================

% ------------------------------------------------------------------
\subsection*{The Bloch--Beilinson--Murre axioms: what is known, and what
Theorem~\ref{thm:main} does and does not add}
% ------------------------------------------------------------------

We first make precise the remark following Theorem~\ref{thm:main}
(Remark~\ref{rem:BBM-significance}) by recalling exactly which of the classical
Bloch--Beilinson--Murre axioms are already established for the filtration built
from the orthogonal tower, and by isolating the one that remains conditional —
since this is the natural place where the convergence results of this thesis
could, in principle, be brought to bear.

Following Peláez (\cite{Pel17}, Def.~6.1.1), for $X\in\Sm/k$ projective a
descending filtration $F^\bullet$ on $\CH^q(X)_\Q$ is of \emph{Bloch--Beilinson--Murre
type} if:
\begin{description}
\item[BBM1] $F^0\CH^q(X)_\Q=\CH^q(X)_\Q$;
\item[BBM2] $F^1\CH^q(X)_\Q=\CH^q_{\mathrm{num}}(X)_\Q$, the cycles numerically
  trivial modulo rational equivalence;
\item[BBM3] $F^\bullet$ is functorial for correspondences between smooth
  projective varieties;
\item[BBM4] for a correspondence $\gamma\in\CH^q(X\times Y)_\Q$ inducing $0$ on a
  fixed Weil cohomology $H$ in the appropriate degree, the induced map
  $\Gr^n_F(\CH^q)$ vanishes (\cite{Pel17}, 6.1.1);
\item[BBM5] $F^{q+1}\CH^q(X)_\Q=0$.
\end{description}
Peláez also isolates a weaker substitute for BBM4 (\cite{Pel17}, 6.1.2):
\begin{description}
\item[BBM4a] if $\gamma\in\CH^q(X\times Y)_\Q$ is \emph{homologically} trivial
  (a strictly weaker hypothesis than in BBM4), then $\Gr^n_F(\CH^q)=0$ for every
  $0\leq n\leq q$.
\end{description}

\begin{theorem}[Peláez, \cite{Pel17}, Thm.~6.1.4 and Rem.~6.1.5]\label{thm:Pel-BBM1235}
The filtration on $\CH^q(X)_R$ obtained by evaluating the orthogonal tower
$bc_{\leq\bullet}$ on $\Z_R$ and mapping in $M(X)(-q)[-2q]$ (as recalled in the
Introduction) satisfies BBM1, BBM2, BBM3, and BBM5, for \emph{every} coefficient
ring $R$, not only $R=\Q$.
\end{theorem}

Condition BBM4 itself is, in Peláez's words, ``in general not much can be said at
the moment'' (\cite{Pel17}, 6.1.6).  What \emph{is} known is the following
conditional result towards the weaker axiom BBM4a.

\begin{conjecture}[Peláez, \cite{Pel17}, Conj.~6.1.7]\label{conj:Pel-multiplicative}
For all $n,m\geq 0$ there exist maps in $DM$
\[
  \mu_{n,m} : (bc_{\leq n}\Z_\Q)\otimes(bc_{\leq m}\Z_\Q)\longrightarrow bc_{\leq n+m}\Z_\Q
\]
compatible with the counits, i.e.\ such that
$\theta_{n+m}\circ\mu_{n,m} = \theta_n\otimes\theta_m$ under the identification
$\Z_\Q\otimes\Z_\Q\cong\Z_\Q$.
\end{conjecture}

\begin{proposition}[Peláez, \cite{Pel17}, Prop.~6.1.8]\label{prop:Pel-BBM4a}
If the maps $\mu_{n,1}$ of Conjecture~\ref{conj:Pel-multiplicative} exist for all
$n\geq 0$, then the filtration of Theorem~\ref{thm:Pel-BBM1235} satisfies BBM4a.
\end{proposition}

Conjecture~\ref{conj:Pel-multiplicative} is a statement about \emph{multiplicative}
compatibility of the tower — that the orthogonal covers $bc_{\leq\bullet}$ can be
made to interact with the tensor structure of $DM$ — and it is logically
independent of the \emph{convergence} statement proved in this thesis.
Theorem~\ref{thm:main} guarantees that, for every $B=M(X)(r)[s]$, the map
$c:\hocolim_n bc_{\leq n}A\to A$ is an isomorphism and the associated filtration
on $\Hom_{DM}(B,A)$ is exhaustive, separated and complete; it says nothing about
whether the covers $bc_{\leq n}$ respect $\otimes$.  Indeed one can check directly
that the argument proving Theorem~\ref{thm:main} never uses the tensor structure
of $DM$ — it only uses that $B$ is compact and orthogonal to
$DM^{\mathrm{eff}}(n)$ for $n\gg 0$ (Proposition~\ref{prop:orthogonality}) — so
it cannot, by itself, produce the maps $\mu_{n,m}$ of
Conjecture~\ref{conj:Pel-multiplicative}.

What Theorem~\ref{thm:main} \emph{does} contribute to this circle of ideas is the
following: it shows that the exhaustive/separated/complete behaviour of the
filtration is not special to the pairing $\Hom_{DM}(\Z_R,\Z_R)$ used in BBM1–BBM5,
but holds uniformly for $\Hom_{DM}(B,A)$ with $B$ \emph{any} shifted Tate twist of
a smooth motive.  In particular, strong convergence supplies, for every such $B$
and every $A$, a strongly convergent spectral sequence
$E_1^{p,q}=\Hom_{DM}(B,(bc_{p/p-1}A)[q-p])\Rightarrow\Hom_{DM}(B,A)$
(Theorem~\ref{thm:main}) against which any candidate multiplicative structure
$\mu_{n,m}$ could be tested for compatibility with differentials, once
constructed by other means.  This suggests the following concrete question, left
open by this thesis:

\begin{itemize}
\item[] \emph{Can the strong convergence of Theorem~\ref{thm:main} be used to
  \textbf{construct} the maps $\mu_{n,m}$ of
  Conjecture~\ref{conj:Pel-multiplicative} — for instance as edge homomorphisms
  of a multiplicative refinement of the spectral sequence of
  Proposition~\ref{prop:SS-exists}, built from a pairing
  $bc_{p/p-1}\otimes bc_{q/q-1}\to bc_{(p+q)/(p+q-1)}$ on associated graded
  pieces — or does it instead clarify an obstruction to their existence?}  A
  first, more modest test case would be to verify Conjecture~\ref{conj:Pel-multiplicative}
  for $\mu_{n,0}$ and $\mu_{0,m}$ (which should follow formally from
  Proposition~\ref{prop:bc-properties}(3)), and then to examine whether the
  half-plane vanishing used in the proof of
  Theorem~\ref{thm:DM-strong-convergence} constrains the bidegree in which
  $\mu_{n,1}$, if it exists, must land.
\end{itemize}

% ------------------------------------------------------------------
\subsection*{An effective bound on the filtration length}
% ------------------------------------------------------------------

The results already established in \S\ref{subsec:convergence-tower} in fact
answer, unconditionally, the natural question of \emph{how large} the
filtration $F_\bullet$ on $\Hom_{DM}(B,A)$ can be.

\begin{proposition}[Effective bound on the filtration length]
\label{prop:effective-bound}
Let $B=M(X)(s)[r]$ with $X\in\Sm/k$ of dimension $d$, and let $A\in DM$ be
arbitrary.  Then, on $H:=\Hom_{DM}(B,A)$, the filtration $F_\bullet$ of
Definition~\ref{def:filtration-F} satisfies
\[
  F_pH = 0 \ \text{ for } p\leq s-1,
  \qquad
  F_pH = H \ \text{ for } p\geq d+s.
\]
In particular $\Gr_F^pH=0$ unless $s\leq p\leq d+s$: the filtration has length
at most $d+1$, depending only on $\dim X$ and not on $A$.
\end{proposition}
\begin{proof}
The vanishing $F_pH=0$ for $p\leq s-1$ is already established in the proof of
Theorem~\ref{thm:DM-strong-convergence} (``Half-plane vanishing''):
$\Hom_{DM}(B,bc_{\leq n}A)=0$ for $n\leq s-1$.

For the upper bound, apply Corollary~\ref{cor:bc-isomorphism}(2)-(3) with
$Y=X$ (so $d=\dim Y$) to $E=B=M(X)(s)[r]$: every $f:B\to A$ admits a unique
lift $g:B\to bc_{\leq d+s}A$ with $\theta_{d+s}^A\circ g=f$, and $f=0$ iff
$g=0$.  This says precisely that
$\theta_{(d+s)\ast}^A:\Hom_{DM}(B,bc_{\leq d+s}A)\to H$ is bijective, so in
particular $F_{d+s}H=H$.

For $n\geq d+s$, the triangle of Proposition~\ref{prop:orthogonal-tower}
gives $\theta_n^A\circ(\iota_{n-1}\circ\cdots\circ\iota_{d+s})=\theta_{d+s}^A$,
so $\theta_{n\ast}^A$ factors the already-surjective map
$\theta_{(d+s)\ast}^A$; a map through which a surjection factors is itself
surjective, so $\theta_{n\ast}^A$ is surjective and $F_nH=H$ for every
$n\geq d+s$.
\end{proof}

\begin{example}
Two extreme cases illustrate Proposition~\ref{prop:effective-bound}.
\begin{itemize}
\item $d=0$ (so $B=\Z(s)[r]$, e.g.\ $X=\operatorname{Spec}k$): the filtration
  has length $1$, jumping directly from $F_{s-1}H=0$ to $F_sH=H$.  This
  recovers, for general $A$, the phenomenon already observed for $A=B$ itself
  in Example~\ref{ex:bc-Tate}.
\item $d=1$ ($X$ a smooth projective curve): the filtration has length at
  most $2$, so $H=\Hom_{DM}(B,A)$ is built from at most two nonzero graded
  pieces, $\Gr_F^sH$ and $\Gr_F^{s+1}H$, glued by a single extension problem
  — no matter how complicated $A$ is.
\end{itemize}
In both cases the bound depends only on $\dim X$, uniformly in $A$; this is
the promised quantitative form of the vanishing used throughout
\S\ref{subsec:convergence-tower}.  (It bounds only the \emph{length} of the
filtration on the abutment, not the page at which the spectral sequence
itself degenerates termwise — that finer question is left open below.)
\end{example}

% ------------------------------------------------------------------
\subsection*{Coefficients: where inverting the characteristic is used}
% ------------------------------------------------------------------

Theorem~\ref{thm:main} is stated, following~\cite{Pel17}, with coefficients in
$R=\Z[\tfrac1p]$, $p$ the exponential characteristic of $k$.  We show this
hypothesis is used at exactly one point of the argument, and can be dropped
whenever $k$ is already perfect.

\begin{proposition}[Integral statement over a perfect field]
\label{prop:integral-perfect}
If $k$ is perfect, Theorem~\ref{thm:main}, Theorem~\ref{thm:DM-strong-convergence},
and Proposition~\ref{prop:orthogonality} all hold with $\Z$-coefficients, i.e.\
without inverting $p$.
\end{proposition}
\begin{proof}
Inspect the proof of Proposition~\ref{prop:orthogonality}.  Its first step
invokes (\cite{Kel13}, Cor.\ 4.13, Thm.\ 4.12, Thm.\ 5.1) to reduce to the
case of a perfect base field; this citation is Kelly's comparison theorem
between motives over $k$ and over a perfect closure of $k$, and it is
precisely this comparison — needed only to move from an arbitrary $k$ to a
perfect one — that requires $p$ to be invertible in the coefficient ring.
When $k$ is already perfect, this reduction step is vacuous: there is
nothing to compare, and the proof proceeds directly from
``If $k$ admits resolution of singularities, by (\cite{VSF00}, Thm.\ 4.3.7)
\ldots'' or, in positive characteristic, directly from (\cite{Sus17},
Thm.\ 5.5.14 and Lem.\ 5.5.6) — neither of which inverts $p$ — to the
vanishing of $\Hom_{DM}(A,\Z)$ for $A\in DM^{\mathrm{eff}}(1)$, which is
checked integrally on generators via the vanishing of motivic cohomology in
negative weights.  Hence Proposition~\ref{prop:orthogonality} holds
integrally once $k$ is perfect.  Since Theorem~\ref{thm:DM-strong-convergence}
and Theorem~\ref{thm:main} are formal consequences of
Proposition~\ref{prop:orthogonality} together with the general machinery of
\S\ref{subsec:orthogonal-tower}--\S\ref{subsubsec:tower-SS}, none of which
refers to the coefficient ring $R$ at all, the same conclusion follows for
both.
\end{proof}

Thus the coefficient hypothesis $R=\Z[\tfrac1p]$ is needed only to cover
imperfect base fields, and only through Kelly's comparison theorem.  Whether
that comparison theorem itself extends to integral coefficients over an
imperfect field is a question about $DM$ over imperfect fields, independent
of the convergence circle of ideas developed in this thesis, and remains
open here.

% ------------------------------------------------------------------
\subsection*{Combining the orthogonal and slice spectral sequences}
% ------------------------------------------------------------------

Corollary~\ref{cor:sliceconv} converges strongly to each individual
$E_1$-term $\Hom_{\SH}(B,s_mG[m+n])$ of the Voevodsky slice spectral
sequence.  We make precise exactly how much can be proved, with the methods
of this thesis, towards assembling this into a single spectral sequence for
$\Hom_{\SH}(B,G)$ itself — correcting what would otherwise be an
overstatement of the strength of these methods.

\begin{proposition}[The outer, slice-indexed spectral sequence]
\label{prop:outer-SS}
Let $G\in\SH$ and $B=\Sigma_T^\infty X_+(s)[t]$, $X\in\Sm/k$.  Applying
$\Hom_{\SH}(B,-)$ to the slice tower $\{f_mG\}_{m\in\Z}$
(Definition~\ref{def:slice-tower}) via Proposition~\ref{prop:tower-SS}
produces, unconditionally, a spectral sequence
\[
  E_1^{m,n} = \Hom_{\SH}(B,s_mG[m+n])
  \;\Rightarrow\;
  \Hom_{\SH}\bigl(B,\hocolim_{m\to-\infty}f_mG\bigr).
\]
By Corollary~\ref{cor:sliceconv}, each individual term $E_1^{m,n}$ is itself
the strongly convergent abutment of the orthogonal spectral sequence on
$s_mG[m+n]$.
\end{proposition}
\begin{proof}
Since $B$ is a compact generator of $\SH$ ($\SH$ is compactly generated with
compact generators $\mathcal{G}_{\SH}$, \S\ref{subsec:SH}), this is a direct
instance of
Proposition~\ref{prop:tower-SS} applied to the tower $A_n:=f_{-n}G$ (with
structure maps the slice-tower maps of Definition~\ref{def:slice-tower} and
associated graded $Q_n=s_{-n}G$), whose hypotheses require nothing beyond
compactness of $B$.  The identification of each $E_1^{m,n}$ with a strongly
convergent abutment is Corollary~\ref{cor:sliceconv}.
\end{proof}

If, in addition, the slice tower converges — i.e.\ the canonical map
$\hocolim_{m\to-\infty}f_mG\to G$ is an isomorphism, a hypothesis about the
slice filtration alone that this thesis does not address — the abutment of
Proposition~\ref{prop:outer-SS} becomes $\Hom_{\SH}(B,G)$, and the outer
spectral sequence converges \emph{weakly} to it
(Theorem~\ref{thm:convergence-criterion}(2)).  It does \emph{not} follow
that this convergence is strong: strong convergence of the tower
$\{f_{-n}G\}_{n\geq 0}$ would require the derived-limit term
$RE_\infty^{m,n}$ of Lemma~\ref{lem:Einf-and-filtration} to vanish, which in
every strongly convergent case established in this thesis
(Theorem~\ref{thm:DM-strong-convergence}, Proposition~\ref{prop:SHconvergence})
is deduced from a half-plane vanishing coming from the source object $B$
being orthogonal to $DM^{\mathrm{eff}}(n)$ (resp.\ $\SH^{\mathrm{eff}}(n)$)
for $n\gg0$ (Note~\ref{note:tower-halfplane}).  The slice tower has no
counterpart of this vanishing: the slices $s_mG$ need not vanish for $m\gg0$
(indeed $s_m\mathbf 1\neq0$ for every $m\geq0$, Proposition~\ref{prop:counterexample}),
so the half-plane argument that proves strong convergence throughout
\S\ref{subsec:convergence-tower} and \S\ref{subsec:SH} does not apply to the
outer tower here.  Consequently, unlike every other result in this section,
assembling the orthogonal and slice spectral sequences into a single
\emph{strongly} convergent spectral sequence for $\Hom_{\SH}(B,G)$ is
\textbf{not} established by this thesis and remains open; what
Proposition~\ref{prop:outer-SS} does establish, unconditionally, is the
weaker (but nontrivial) fact that the two constructions fit into a single
formal framework, with the inner strong convergence of
Corollary~\ref{cor:sliceconv} feeding into each page of the outer one.

% ------------------------------------------------------------------
\subsection*{Necessity of the hypothesis in $\SH$}
% ------------------------------------------------------------------

Proposition~\ref{prop:SHconvergence} shows that $A\in\SH^\perp(r)$ for some
$r\in\Z$ is \emph{sufficient} for strong convergence on $A$, and
Proposition~\ref{prop:counterexample} shows the hypothesis is not vacuous,
since it already fails for the unit $\mathbf 1$.  Of the questions recorded
in this section, this is the one we have been least able to make progress
on: unlike the filtration-length bound above, membership in some
$\SH^\perp(r)$ is a condition on \emph{all} weights of $A$ at once, and the
argument of Proposition~\ref{prop:SHconvergence}(1) (that the tower
stabilises once every $\theta_m^A$, $m\geq r$, is simultaneously an
isomorphism) does not obviously admit a partial or asymptotic version.  We
leave open whether membership in some $\SH^\perp(r)$ is also necessary for
strong convergence, or whether a strictly larger class — for instance
objects whose slices $s_mA$ vanish outside a sparse, but unbounded, set of
weights — already suffices.

% ------------------------------------------------------------------
\subsection*{Concluding remarks}
% ------------------------------------------------------------------

The convergence of the orthogonal spectral sequence, established
unconditionally in Theorem~\ref{thm:main} for every $B=M(X)(r)[s]$, was the
central question motivating this thesis.  The results assembled since —
the failure and partial recovery of convergence in $\SH$
(\S\ref{subsec:SH}), the explicit filtration-length bound and its
worked examples, the precise isolation of where integral coefficients are
available (Proposition~\ref{prop:integral-perfect}), the unconditional
outer spectral sequence of Proposition~\ref{prop:outer-SS}, and the three
applications of \S\ref{subsec:applications} identifying the first two steps
of the orthogonal filtration on Chow groups with numerical and incidence
equivalence — together show that the convergence statement proved here is
not an isolated technical fact, but the load-bearing property that makes
the orthogonal tower usable as a Bloch--Beilinson--Murre filtration in the
first place.  What remains open — Peláez's multiplicativity
Conjecture~\ref{conj:Pel-multiplicative} towards BBM4a, necessity of the
$\SH^\perp(r)$ hypothesis, and strong convergence of the slice tower itself
— is left open deliberately: each is a genuine unsolved problem in the
current literature, and none is resolved by the methods developed here.
...
\bibitem{GalPel22}
C.\ Galindo, P.\ Peláez. On the convergence of the orthogonal spectral sequence.
\textit{Homology Homotopy Appl.} \textbf{24}(2):389--401, 2022.

\bibitem{Gri71}
P.A.\ Griffiths. Some transcendental methods in the study of algebraic cycles.
In \textit{Several Complex Variables, II} (Proc.\ Internat.\ Conf., Univ.\
Maryland, College Park, Md., 1970), Lecture Notes in Math.\ \textbf{185},
pp.\ 1--46. Springer, Berlin--New York, 1971.

\bibitem{HerPel25}
P.\ Peláez. Regular homomorphisms and mixed motives.
Preprint, arXiv:2509.15920 (2025), to appear in \textit{St.\ Petersburg Math.\
J.}

\bibitem{Lie68}
D.I.\ Lieberman. Higher Picard varieties. \textit{Amer.\ J.\ Math.}
\textbf{90}:1165--1199, 1968.

\bibitem{Pel25}
P.\ Peláez. Some remarks on Chow correspondences. Preprint, 2025.

\bibitem{PelRey25}
P.\ Peláez. Incidence equivalence and the Bloch--Beilinson
filtration. Preprint, arXiv:2501.19147 (2025).

\bibitem{Sai79}
H.\ Saito. Abelian varieties attached to cycles of intermediate dimension.
\textit{Nagoya Math.\ J.} \textbf{75}:95--119, 1979.

\bibitem{Sam60}
P.\ Samuel. Relations d'équivalence en géométrie algébrique. In
\textit{Proc.\ Internat.\ Congress Math.\ 1958}, pp.\ 470--487.
